...
|5|**Expansión descontrolada del alcance**; requerimientos no planificados a mitad de desarrollo.|Establecer una definición estricta del Producto Mínimo Viable (MVP). Implementar un Comité de Control de Cambios que evalúe y priorice cada solicitud nueva contra el presupuesto.|Moderado|
...
|10|**Fallas en la integración con sistemas legados**; sistemas antiguos sin conectividad moderna (APIs).|Desarrollo de una capa de Middleware (API Gateway) que actúe como buffer entre el nuevo CRM y los sistemas antiguos, evitando consultas directas a las bases de datos.|Alto|
